...
const DeviceIcon = ({ type }: { type: string }) => {
  switch (type.toUpperCase()) {
    case "TV": return <TvIcon className="w-6 h-6" />;
    case "LIGHT": return <LightBulbIcon className="w-6 h-6" />;
    case "AC": return <CpuChipIcon className="w-6 h-6" />;
    default: return <CommandLineIcon className="w-6 h-6" />;
  }
};
